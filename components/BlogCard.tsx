...
function formatDate(dateStr: string) {
  ...
  return date.toLocaleDateString("en-US", {
    ...
    month: "short",
    day: "numeric",
    timeZone: "UTC" // Prevents server/client hydration mismatch
  });
}
    ...
    >
      ...
      <div className="relative w-full aspect-video bg-[#1a0f0a] overflow-hidden">
        {featuredImage?.source_url ? (
          <Image
            src={featuredImage.source_url}
            alt={featuredImage.alt_text || post.title?.rendered || "Blog Post"}
            fill
            sizes="(max-width: 768px) 100vw, 33vw"
            className="object-cover group-hover:scale-105 transition-transform duration-500"
          />
      ...
      <div className="flex flex-col gap-3 p-5 flex-1">
        {/* Title */}
        <h2
          className="text-base font-semibold text-stone-100 leading-snug tracking-tight"
          dangerouslySetInnerHTML={{ __html: post.title?.rendered || "" }}
        />
