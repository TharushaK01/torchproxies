      ...
      >
        <div className="relative" style={{ width: cardWidth, height: cardHeight }}>
          {items.map((item, i) => {
            ...
            // MODIFIED: Only show cards starting from active index (rawOffset >= 0) up to visibleCards limit.
            // This completely hides any cards to the left (negative offsets).
            const isVisible = rawOffset >= 0 && rawOffset < visibleCards;
            if (!isVisible) return null;

            const distance = rawOffset; // Since rawOffset is strictly >= 0
            const opacity = 1 - distance * falloff;
            const scale = 1 - distance * 0.08;
            const translateX = rawOffset * spread;
            const translateZ = -distance * depth;
